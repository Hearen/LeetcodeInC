...
//return the actual size of the random array - at least 10;
int randomIntArray(int** array, int size, int low, int high)
{
    srand(time(NULL));
    int size0 = rand()%size+10;
    *array = (int*)malloc(sizeof(int)*size0);
...
void mergeAB(int *a, int *aux, int l, int m, int r)
{
    ...
    int k;
    for(k = l; k <= r; k++)
        a[k] = ((i<=m && aux[i]<=aux[j]) || j==r+1)? aux[i++] : aux[j++];
}
