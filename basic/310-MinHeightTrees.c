...
int* findMinHeightTrees(int n, int** edges, int rSize, int cSize, int* returnSize)
{
    ...
    *returnSize = 0;
    ...
    int next = 0;
    while(count > 2) //this condition is quite essential;
    {
        for(int i = 0; i < *returnSize; i++)
        {
            int end = arr[i];
            count--; //remove the leaf then the corresponding edges count should be decremented;
            for(int k = 0; k < colSizes[end]; k++) //decrement related vertexes in degrees;
            {
                int begin = graph[end][k];
                degrees[begin]--;
                if(degrees[begin] == 1) //when the degree of the connected vertex is 1, it turns leaf now;
                    nextLevel[next++] = graph[end][k]; //collect the leaves for the next round;
            }
        }
