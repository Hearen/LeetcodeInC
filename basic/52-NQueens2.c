...
void traverse(int row, int size, bool* col, bool* fslash, bool* bslash, int* count)
{
    if(row == size)
    {
        (*count)++;
    ...
    for(int j = 0; j < size; j++)
    {
        if(col[j] && fslash[row+j] && bslash[row+size-j])
        {
            ...
            traverse(row+1, size, col, fslash, bslash, count);
...
int totalNQueens(int n)
{
    ...
    int count = 0;
    traverse(0, n, col, fslash, bslash, &count);
    return count;
}
