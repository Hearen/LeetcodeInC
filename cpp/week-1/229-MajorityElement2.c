...
int* majorityElement0(int* nums, int size, int *returnSize)
{
    ...
    {
        if((count1 == 0 || nums[i] == a) && b!=nums[i]) //ensure count first before creating new
            count1++, a = nums[i];
...
//count first then consider set a new one if cannot count ->
//but if no new can be created, un-count them all;
