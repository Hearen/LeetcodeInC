...
public:
    ...
    {
        vector<string> v;
        int size = words.size(),begin=0,end=1,length;
        while(begin<size)
        {
            string s=words[begin];
            int length = s.size();
            while(end<size && length+1+words[end].size()<=maxWidth) length += 1+words[end++].size();
            int count = end - begin - 1;
            if(end<size && count>0)
            {
                int p = (maxWidth-length)/count,q = (maxWidth-length)%count;
                for(int i=1;i<=count;i++) s += string(p+(i>q?1:2),' ') + words[begin+i];
            }
            else
            {
                for(int i=1;i<=count;i++) s += ' '+words[begin+i];
                s += string(maxWidth-length,' ');
            }
            v.push_back(s);
            begin = end++;
        }
