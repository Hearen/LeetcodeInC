...
Reference   : https://leetcode.com/discuss/91599/far-beating-submissions-accepted-with-16ms-well-explained
*******************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
void helper0(char* num, int len, int start, int target, long long sum, long long pre, char* stack, int top, char*** arr, int* returnSize)
{
    ...
    for(int i = start; i < len; i++)
    {
        ...
        {
            stack[top+1] = num[i];
            helper0(num, len, i+1, target, val, val, stack, top+1, arr, returnSize);
        }
        else
        {
            stack[top+2] = num[i];
            stack[index] = '-'; //get the sum directly by sum-val and record -1*val as pre in case the next operator is '*';
            helper0(num, len, i+1, target, sum-val, -1*val, stack, top+2, arr, returnSize);
            stack[index] = '+'; //store val as pre in case the next operator is '*';
            helper0(num, len, i+1, target, sum+val, val, stack, top+2, arr, returnSize);
            ...
            helper0(num, len, i+1, target, sum-pre+pre*val, pre*val, stack, top+2, arr, returnSize);
        }
        if(num[start] == '0') break; //no number starts with 0;
        top++;
    }
}
char** addOperators0(char* num, int target, int* returnSize)
{
    ...
    *returnSize = 0;
    ...
    int top = -1;
    helper0(num, len, 0, target, 0, 0, stack, top, &arr, returnSize);
    return arr;
}
