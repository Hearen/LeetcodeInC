...
{
    for(int i = 0; i < sizeof(unitsN)/sizeof(int); i++) //only this part should be handled for several times which will split the number into several section;
    {
        if(*num >= unitsN[i])
        {
            int t = *num/unitsN[i];
    ...
    for(int i = sizeof(tensN)/sizeof(int)-1; i > -1; i--) //only be handled once for each number;
    {
        ...
        {
            int t = *num - tensN[i];
            if(*s) //in case that the tens is the number, avoid redundant preceding  white space;
                strcat(s, " ");
            strcat(s, tens[i]);
            *num -= tensN[i]; 
            break;
    ...
    for(int i = sizeof(specialsN)/sizeof(int)-1; i > -1; i--) //only be handled once for each number;
    {
        if(*num == specialsN[i])
        {
            if(*s) //in case that the special is the number, avoid redundant preceding white space;
                strcat(s, " ");
            strcat(s, specials[i]);
            break;
        }
